...
<h1>Informe de Nota de credito </h1>
...
<option value="ENTREGA" <?php echo $ENTREGA1;?> >Pago</option>
</select>
</td>
...
<td ><input placeholder="Nota Credito" class="textbox" name="txt_factura" id="txt_factura" type="text" value="<?php echo $factura ?>" /></td>
...
<table rules='all' frame='box' class='nota-table fixed'>
<?php
...
$result1 = mysql_query($query_registro1, $conn) or die(mysql_error());
 while($row = mysql_fetch_array($result1))
  ...
  if($FECHA_REALIZACION  == date("Y-m-d"))
  {
	...
	echo  "<td align='center'  id='hoy'> <a href=DescripcionServicio.php?CODIGO_SERVICIO=" .urlencode($CODIGO_SERVICIO). "&CODIGO_PROYECTO=" . urlencode($CODIGO_PROYECTO). ">Editor</a>
           </td>";
	...
	if($ESTADO == "OK")
	{
    echo  "<td align='center' id='hoy' >".($FECHA_ENTREGA)."</td>";
	}
	else
	{
	if($FECHA_ENTREGA > $fecha7)
	{
    echo  "<td align='center' id='hoy' >".substr($FECHA_ENTREGA,0,11)."</td>";
	}
