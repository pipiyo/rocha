...
<head>
  <title>Informe Presentacion V1.0.0</title>
  ...
  <script language = javascript>
    $(function(){
     $('.tabla-presentacion').tablesorter(); 
  });
  </script>
...
<table class="tabla-presentacion fixed">
<?php
...
$query_registro =    "SELECT * FROM servicio WHERE CODIGO_RADICADO IN (    SELECT CODIGO_RADICADO FROM radicado WHERE EJECUTIVO IN(".$consulta.")    )   AND  ".$ESTADOV." AND  NOMBRE_SERVICIO =  'Presentacion'  AND  FECHA_".$buscaf." between '".$txt_desde."' and '".$txt_hasta."' " ;	
$query_registro = ( empty($_GET["buscar_codigo"]) ) ?   "SELECT * FROM servicio WHERE  CODIGO_RADICADO IN (    SELECT CODIGO_RADICADO FROM radicado WHERE EJECUTIVO IN(".$consulta.")    ) AND  ".$ESTADOV."  AND  NOMBRE_SERVICIO =  'Presentacion' and  FECHA_".$buscaf." between '".$txt_desde."' and '".$txt_hasta."'  "   :   "SELECT * FROM servicio WHERE CODIGO_RADICADO = '".$_GET['buscar_codigo']."' AND  ".$ESTADOV."   AND  NOMBRE_SERVICIO =  'Presentacion' and  FECHA_".$buscaf." between '".$txt_desde."' and '".$txt_hasta."'  "
  ;	
} else {

$query_registro =    "SELECT * FROM servicio WHERE NOT CODIGO_RADICADO = '' AND  ".$ESTADOV." AND  NOMBRE_SERVICIO =  'Presentacion'  AND  FECHA_".$buscaf." between '".$txt_desde."' and '".$txt_hasta."' " ;	
$query_registro = ( empty($_GET["buscar_codigo"]) ) ?   "SELECT * FROM servicio WHERE NOT CODIGO_RADICADO = '' AND  ".$ESTADOV."  AND  NOMBRE_SERVICIO =  'Presentacion' and  FECHA_".$buscaf." between '".$txt_desde."' and '".$txt_hasta."'  "   :   "SELECT * FROM servicio WHERE CODIGO_RADICADO = '".$_GET['buscar_codigo']."' AND  ".$ESTADOV."   AND  NOMBRE_SERVICIO =  'Presentacion' and  FECHA_".$buscaf." between '".$txt_desde."' and '".$txt_hasta."'  "
  ;	
...
$FECHA_VARIABLE ="";
 while($row = mysql_fetch_array($result))
  {  
	...
	echo  "<td id='hoy'>  <a href=DescripcionServicioRadicado.php?CODIGO_SERVICIO=" .urlencode($CODIGO_SERVICIO). "&CODIGO_PROYECTO=" . urlencode($CODIGO_PROYECTO). "&editar=Presentacion>" .$CODIGO_SERVICIO . "</a></td>";
  ...
  else
  {
	...
	echo  "<td><a href=DescripcionServicioRadicado.php?CODIGO_SERVICIO=" .urlencode($CODIGO_SERVICIO). "&CODIGO_PROYECTO=" . urlencode($CODIGO_PROYECTO). "&editar=Presentacion>" .$CODIGO_SERVICIO . "</a></td>";
