...
<head>
  <title>Bodega Quiebre Disponible V1.2.0</title>
...
<center>  <h1>Consulta Stock Disponible </h1>   
...
$query_registroX = "SELECT * FROM PRODUCTO where CODIGO_PRODUCTO  IN('".implode("','",$QUEBRADOS)."') and not TEMPORADA = '2' order by DESCRIPCION ASC limit ".$CORTEy.",".$CORTEx."";
...
$query_registroXX = "SELECT sum(oc_producto.CANTIDAD_RECIBIDA) as trans,sum(oc_producto.CANTIDAD) as trans1  FROM oc_producto, producto, orden_de_compra WHERE orden_de_compra.CODIGO_OC = oc_producto.CODIGO_OC and oc_producto.CODIGO_PRODUCTO = producto.CODIGO_PRODUCTO and producto.CODIGO_PRODUCTO = '".$CODIGO_PRODUCTO."' and orden_de_compra.ESTADO = 'EN PROCESO'";
$result1X = mysql_query($query_registroXX, $conn) or die(mysql_error());
...
$query_registro3X = "select sum(producto_vale_emision.CANTIDAD_ENTREGADA) as valor,sum(producto_vale_emision.CANTIDAD_SOLICITADA) as valor1  from vale_emision, producto_vale_emision, producto where vale_emision.
COD_VALE = producto_vale_emision.CODIGO_VALE and producto_vale_emision.CODIGO_PRODUCTO = producto.CODIGO_PRODUCTO and producto.CODIGO_PRODUCTO = '".$CODIGO_PRODUCTO."'  and vale_emision.ESTADO = 'PENDIENTE'";
...
$query_registroX = "SELECT * FROM PRODUCTO where CODIGO_PRODUCTO  IN('".implode("','",$QUEBRADOS)."') and not TEMPORADA = '2' order by DESCRIPCION ASC LIMIT 500";
$resultX = mysql_query($query_registroX, $conn) or die(mysql_error());
 while($row = mysql_fetch_array($resultX))
  {
	$CODIGO_PRODUCTO = $row["CODIGO_PRODUCTO"];
...
{
?>
<a  class="noactivo"   onClick="document.formulario.action='ProductoQuiebreDisponible.php?pagina=<?php echo $i;?>&buscar_codigo=<?php echo $BUSCAR_CODIGO;?>&BUSCAR_DESCRIPCION=<?php echo $BUSCAR_DESCRIPCION;?>'; document.formulario.submit();"
><?php echo "...";?></a>
<?php
}
?>
...
{
?>
<a  class="activo" onClick="document.formulario.action='ProductoQuiebreDisponible.php?pagina=<?php echo $i;?>&buscar_codigo=<?php echo $BUSCAR_CODIGO;?>&BUSCAR_DESCRIPCION=<?php echo $BUSCAR_DESCRIPCION;?>'; document.formulario.submit();"
><?php echo $i;?></a>
<?php
}
