...
<html>
    ...
    <title>Editar cotizacion V1.0</title>
...
<body>
  ...
  <input type="hidden" value=<?php echo $rad; ?> id="codigo-radicado">
	...
	<form method="post" action="editar-cotizacion.php" enctype="multipart/form-data" id="testform">
		<?php
...
while($row = mysql_fetch_array($result1))
{
  echo "<tr id='producto-general".$indice."' class='producto-general'><th align='center' colspan='7'>".$row["UBICACION"]."</th><th><a id='eliminar-cot".$indice."' class='eliminar-cot'><i class='fa fa-trash-o fa-2x'></i></a></th></tr>";
  ...
  {
    echo "<tr  id='fila".$indice2."' class='filcount' ><td align='right' align='right' id='cantidaditem".$indice1."' class='cantidaditem'>".$row["CANTIDAD"]."</td><td align='center' id='codigo_producto".$indice1."'>".$row["CODIGO_PRODUCTO"]."</td>";
    echo "<td>".$row["DESCRIPCION"]."<input type='hidden' value=".$row["CANTIDAD"]." id='valorantiguocantidad".$indice1."'> <input type='hidden' value='".number_format($row["VALOR_UNITARIO"],0,",",".")."' id='valorcantidad".$indice1."'> <input type='hidden' id='valorantiguo".$indice1."'></td>";
    $ms = "";
    $ls = "";
    $es = "";
    if($row["POM"] == "m")
    ...
    if($row["POM"] != "")
    {
      echo "<td> <select class='seleccion' id='seleccion".$indice1."'><option>Seleccione</option><option $ms value='m'>Melamina</option> <option $ls value='l'>Laminado</option> <option $es value='e'>Enchape</option> </select> </td>";
    }
    else
    {
      echo "<td></td>";
    }
    echo "<td align='right' id='unitario".$indice1."'>".number_format($row["VALOR_UNITARIO"],0,",",".")."</td>";
    echo "<td align='center' id='descuento".$indice1."' class='descuento'>".number_format($row["DESCUENTO"],0,",",".")."</td>";
    echo "<td align='right' id='total-final".$indice1."'>".number_format($row["VALOR_TOTAL"],0,",",".")."</td>";
    echo "<td align='right' id='costo".$indice1."'>".number_format($row["COSTO"],0,",",".")."</td>";
    echo "<td align='center'><i id='basura".$indice1."' class='fa fa-trash-o fa-2x bas'></i></td>";
    echo "<td style='display:none;' align='center'>".$row["ID"]."</td></tr>";
    $suma += $row["VALOR_TOTAL"];
    $indice1++;
    $indice2++;
  }
  echo "<tr id='fila-total".$indice."' class='fila-total'><td></td> <td></td> <td></td> <td></td> <td></td> <td align='right'><strong>Total</strong></td> <td align='right'><b>".number_format($suma,0,",",".")."</b></td> <td align='right'></td></tr>";
  $indice++;
}
mysql_free_result($result1);
...
?> 
  ...
  <div class="content-preview">
   <input style=""  type="button" class="boton" name = 'preview1' id='preview1' value='Ingresar' /> 
  </div>
