...
$(document).ready(function() {
  ...
  function goheadfixed(classtable) {
    ...
    if($(classtable).length) {
      ...
      $('.fix-head').css({'position':'absolute', 'overflow':'hidden', 'top': hcaption+'px', 'left':0, 'z-index':-1});
    
      $(window).scroll(function () {
        var vscroll = $(window).scrollTop();
  
        if(vscroll >= hinner + hcaption - 140)
          $('.fix-head').css({'top':(vscroll-hinner)+'px','margin-top':'150px', 'z-index':2});
        else
          $('.fix-head').css({'top': hcaption+'px','margin-top':'0px', 'z-index':-1});
      });
