...
require_once('../Conexion/Conexionpoo.php'); 
	/**
	* 
	Query ingreso aglomerado 
	*/
	class Query_ingreso_aglomerado extends Conexionpoo
	{
	...
	*/
		public function Ingreso_productos($productos, $generico)
		{
			...
			$this->query = $this->con->prepare("INSERT INTO `producto` (CODIGO_PRODUCTO, DESCRIPCION, CATEGORIA, CODIGO_GENERICO, TEMPORADA, FAMILIA, CUERPO, FRENTE, CANTO, TRASCARA, ESPESOR, RUTA, RUTA1, RUTA2, CAD_2D, CAD_3D) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	*/
		public function Superficies_colores($cat, $cod, $des)
		{
			...
			$descripcion_productos = array();
			$superficies_nombre = array();
			$superficies_alias = array();

			$superficies = $this->con->query("SELECT `superficies`.`id_superficies`, `superficies`.`nombre`, `superficies`.`alias`, `categoria_producto`.`cuerpo`, `categoria_producto`.`frente`, `superficies_categoria_producto`.`id_superficies_categoria` 
FROM `superficies`, `superficies_categoria_producto`, `categoria_producto` 
where `categoria_producto`.`id_categoria_producto` = '$cat' AND `superficies_categoria_producto`.`id_categoria_productos` = `categoria_producto`.`id_categoria_producto` And `superficies`.`id_superficies` = `superficies_categoria_producto`.`id_superficies`");

			while($fila = $superficies->fetch_assoc()){
				$regla_muebles = ( $fila['frente'] == '1' and $fila['cuerpo'] == '1') ? true : false ;
				$a .= ($a == "(") ? "" : "," ;
				$a .= "'".$fila['id_superficies']."'"; 

				$id_sup_cat .= ($id_sup_cat == "(") ? "" : "," ;
			...
			$lista_final = $this->Concatenar_colores($regla_muebles, $codigo_productos, $des, $a, $cat, $id_sup_cat);
